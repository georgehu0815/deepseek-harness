...
新增的核心字段是 [`mcp-client` `allowedTools`](../../../../packages/mcp/mcp-client/README.zh.md)(有其自己的[笔记](2026-08-28-mcp-client-tool-allowlist.zh.md)):Claude Code 服务器的 `tools:` 白名单在 `mcp-client` 上此前没有表达方式,而在连接时过滤能让不允许的工具不进入注册表与模型。
...
- [mcp-client 工具白名单](2026-08-28-mcp-client-tool-allowlist.zh.md) —— 本特性依赖的新增核心字段。
- [mcp-client 插件](2026-07-07-mcp-client-plugin.zh.md) —— 生成行所针对的 MCP 接缝。
