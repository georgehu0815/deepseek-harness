...
[Client gateway](../../../../packages/api/gateway/README.zh.md) 用返回不可用状态的 `Service.check` 谓词准备 namespace fiber，同步安装整个贡献项的直接方法和作用域方法，然后通过 Cordis 通知发布新 namespace 的就绪状态。扩展会复用已有 namespace Service，且不会在方法安装之间让出执行权；已有消费方不会重启。
...
[Remote 方法架构](../architecture/2026-08-02-typert-remote-method-calls.zh.md)与[生成契约的构建顺序](../process/2026-08-08-api-remotes-generated-contract-build.zh.md)仍是有效依据，未被取代；本决策在该设计内补充 namespace 的就绪保证。
