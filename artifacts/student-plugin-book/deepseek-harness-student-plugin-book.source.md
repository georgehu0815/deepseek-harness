...
flowchart TB
  subgraph HOST["Host layer"]
    Client["User / Web / headless / ACP / SDK"]
  end
  subgraph FRAMEWORK["Framework — @deepseek-ai/cordis (vendored)"]
    Loader["Loader — reads cordis.yml"]
    Fibers["Fiber tree — one fiber per plugin"]
    Bus["Typed event bus — ctx.on / ctx.emit"]
    Effects["Effect registry — reversible resources"]
  end
  subgraph LIBRARY["Library packages — @deepseek-ai/dsh-*"]
    Agents["agents service (dsh-agent)"]
    Tools["tools service (dsh-tools)"]
    Sessions["sessions service (dsh-session)"]
    Prompt["systemPrompt service"]
  end
  subgraph CUSTOMER["Customer plugin — your code"]
    Student["student-plugin"]
    Stats["studentStats service"]
  end
  ...
  Fibers --> Sessions
  Fibers --> Student
  Agents -- "emits agent/* events" --> Bus
  Bus -- "delivers events" --> Student
  Student -- "ctx.plugin()" --> Stats
  Student -- "reads / records" --> Stats
  Effects -. "manages lifetime of" .- Student
```
...
```mermaid
flowchart LR
  Config["cordis.yml rows"] --> Loader["Cordis Loader"]
  Loader --> Ctx["ctx : Context (your fiber's handle)"]
  Ctx --> Inject["ctx.inject(deps, child => ...)"]
  Ctx --> Plugin["ctx.plugin(Service|fn)"]
  Ctx --> On["ctx.on(event, handler)"]
  Ctx --> Effect["ctx.effect(() => disposer)"]
  Ctx --> Emit["ctx.emit(event, payload)"]
  Plugin --> Svc["StudentStatsService"]
  Inject --> Child["child ctx with studentStats available"]
  Child --> On
  On --> Handler["listener body reads Svc, emits summary"]
  Effect --> Disposer["cleanup runs on unload"]
```
...
sequenceDiagram
  ...
  participant F as Framework (Cordis)
  participant P as Your plugin
  participant S as Injected services
  L->>F: read cordis.yml row for the plugin
  F->>F: PENDING — wait for inject deps
  S-->>F: required services published
  F->>P: apply(ctx)  (LOADING)
  P->>F: ctx.plugin / ctx.on / ctx.effect (register)
  F->>P: ACTIVE — events dispatch, effects live
  Note over F,P: ...plugin runs, listeners fire...
  F->>P: UNLOADING (dep vanished, HMR, or shutdown)
  P->>F: disposers run in reverse order
  F->>F: DISPOSED — no trace remains
```
