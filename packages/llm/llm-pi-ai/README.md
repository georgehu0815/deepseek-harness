...
Set `strictToolSchemas` for a provider that requires OpenAI strict tool schemas. The shipped `agency-copilot-gpt` route enables this mode. For each schema that pi-ai can constrain without changing its meaning, the adapter sends every optional scalar as a nullable branch, marks all properties required, requests strict constrained sampling, and removes only the introduced null placeholders from returned arguments before tool execution. The canonical Harness schema remains unchanged, so omitted arguments, accepted null values, and sandbox checks keep their original meaning. Schemas with any `oneOf`, an authored `type` union containing `object` or `array`, an optional object or array that would require a structured union, or a pre-existing `anyOf` object or array branch are sent unchanged and non-strict; required objects and arrays remain strict-encodable.
