...
The package root exposes the Cordis plugin contract, `PiAiAdapter`, `supportedProtocols()`, `AGENCY_COPILOT_PROVIDER`, and `DEFAULT_COPILOT_MODEL`; profile resolution, catalog materialization, provider construction, replay conversion, and stream conversion remain package-internal.

## Config

Configure credentials, the model catalog, and deployment-specific transport settings per provider, keyed by the provider route itself. The base bundle declares `agency-copilot` with the Anthropic Messages protocol, `CLAUDE_CODE_COPILOT_TOKEN`, the required Copilot integration headers, and the Claude catalog `claude-sonnet-5`, `claude-opus-5`, and `claude-opus-4-8`; this is the shipped default route and remains overridable through the `llm-pi-ai` settings section. Because one route carries one wire protocol, the shipped GPT models live on a second route `agency-copilot-gpt` with the OpenAI Responses protocol (`openai-responses`); it reuses the same Copilot enterprise endpoint and `CLAUDE_CODE_COPILOT_TOKEN` credential. It enables strict tool-schema transport so optional Harness arguments are required and nullable on the provider wire, then removes only those null placeholders before tool execution; this preserves omitted arguments without weakening the canonical schemas or sandbox checks. The route offers `gpt-5.6-sol`, `gpt-5.6-terra`, `gpt-5.6-luna`, and `gpt-6-astra`. Selecting any of these from the Models dropdown routes chat through that route. Each profile may set a `retryPolicy`; omission uses normal mode with five retries. `apiKeyEnv` is the exact credential reference that the adapter requests from `ctx.credentials` per operation, so no secret enters this file. The mounted credentials provider interprets that reference; the shipped [`credentials-local`](../../credentials/credentials-local/README.md) provider can resolve Agency Copilot environment aliases and the signed-in `copilot-cli` OS credential only after its exact-reference sources miss. This adapter does not know those aliases or credential stores. Omitting `apiKeyEnv` leaves the route unauthenticated, which for an installed catalog route means pi-ai's provider-native ambient discovery; a configured reference that resolves to nothing fails the request with `MISSING_CREDENTIAL` instead, because falling through would authenticate with whatever unrelated key the environment happens to hold. One credential serves every model on its route.

`authMode` controls how an explicitly resolved credential reaches the endpoint. Its default, `api-key`, uses the protocol's API-key option; `bearer` sends `Authorization: Bearer <credential>` instead. The shipped Agency Copilot profile selects `bearer` because its Anthropic Messages endpoint accepts a Copilot access token rather than an Anthropic API key.
...
## Verifying a route's models

A route's `api`, its model `id`s, and each model's declared `input` are claims about an endpoint, and configuration parsing cannot check any of them. A unit test over the patch file proves the YAML parses and nothing more, so verify a new or edited route in four widening steps, each of which can fail where the previous one passed.

1. **Parse the configuration.** For the shipped catalog this is `npx vitest run packages/bundle/base/tests/base.spec.ts`, which rejects malformed YAML and drift between the patch and its expected config.
2. **Reach the endpoint.** Send one minimal request per model to the protocol's own path — `/v1/messages` for `anthropic-messages`, `/v1/responses` for the Responses protocols — carrying the route's `headers` and its credential. A model id the endpoint does not serve fails here. Read the model id back out of the response: an endpoint that answers with a substituted id has silently rerouted the request, which a status code alone does not reveal.
3. **Run the adapter.** Endpoint success exercises neither this adapter nor the credential seam. Point `agent-default-model` at the route with a patch overlay and run one prompt through an assembled profile. Confirm the override landed — `dsh --profile headless --patch <file> --dump-config` prints the composed row — because a malformed patch only warns and is skipped, leaving an unverified run to answer from the default model instead.
4. **Exercise tools and images.** A model that returns plain text can still fail the work the harness asks of it. Drive one tool call per model, and for every model declaring `input: [text, image]`, send an image and have the model describe it; that declaration is otherwise an unchecked assertion.

Step 3 writes under `$DSH_HOME`, outside a workspace sandbox.
