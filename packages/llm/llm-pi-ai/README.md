...
Set `strictToolSchemas` for a provider that requires OpenAI strict tool schemas. The shipped `agency-copilot-gpt` route enables this mode. For each schema that pi-ai can constrain without changing its meaning, the adapter sends every optional scalar as a nullable branch and every exact-type optional array as a nullable type, marks all properties required, requests strict constrained sampling, and removes only the introduced null placeholders from returned arguments before tool execution. The canonical Harness schema remains unchanged, so omitted arguments, accepted null values, and sandbox checks keep their original meaning; schemas with any `oneOf`, optional objects that would require a structured union, or pre-existing `anyOf` object or array branches are sent unchanged and non-strict.
