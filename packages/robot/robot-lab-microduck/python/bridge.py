...
def trainer_provenance(spec):
    ...
    value = {"backend": backend, "learnerDevice": "metal" if backend == "mlx" else "cpu",
...
def bridge_hash():
    """Fingerprint bridge and studio runtime code without importing optional learners."""
    return digest({name: file_hash(Path(__file__).with_name(name)) for name in ("bridge.py", "studio.py")})
...
def validate_spec(spec, limits):
    ...
    if spec["backend"] not in ("cpu", "mlx"):
        raise ValueError("training backend must be cpu or mlx")
...
class Bridge:
    ...
    def run(self, run_id):
        ...
        if backend not in ("cpu", "mlx") or trainer["backend"] != backend:
        ...
        expected_helpers = {"mlx_ppo.py"} if backend == "mlx" else set()
    ...
    def prepare_train(self, run_id, spec):
        ...
        trainer = trainer_provenance(spec)
        path = self.path(run_id)
    ...
    def configure_run(self, run, training=False):
        ...
        for name, checksum in trainer["helperSha256"].items():
            if name != "mlx_ppo.py" or file_hash(Path(__file__).with_name(name)) != checksum:
                raise ValueError("Python learner/exporter differs from frozen provenance")
        if training and trainer != trainer_provenance(run["spec"]):
                ...
                def create():
                    with frozen_bam(run["provenance"]["bam"]):
                        return BehaviorEnv(behavior_id, weight_overrides=weight_overrides,
                                           ...
                                           action_delay=True, standing_spawns=True, spotter=False)
                return create
    ...
    def rollout(self, loaded, physics, steps, seed, command, frames):
        import numpy as np
        ...
        try:
            ...
            for step in range(steps):
                ...
                if terminated or truncated:
                    break
            return {"seed": seed, "steps": step + 1, "terminated": bool(terminated), "bamSettings": settings,
                    "reward": total_reward, "uprightFraction": float(np.mean(upright)),
                    "poseRmse": float(math.sqrt(np.mean(pose_error))) if pose_error else None}, recorded
        finally:
            env.close()
    ...
    def evaluate(self, spec):
        ...
        episodes = [self.rollout(loaded, physics, steps, seed + ep, [0, 0, 0], False)[0] for ep in range(n)]
        report = {**admission, "episodes": episodes,
                  ...
                  "limitations": ["Local deterministic BAM evaluation, not hardware approval.",
                                  ...
                                  "No domain-randomization or physical trial was performed."], "evaluatedAt": now()}
...
def main():
    ...
    with contextlib.redirect_stdout(sys.stderr):
        if op == "studio":
            ...
            result = {"operation": "train", "run": bridge.prepare_train(request["runId"], request["spec"])}
        elif op == "train":
