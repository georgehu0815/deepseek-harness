...
import type { RobotDanceCriteria } from '@deepseek-ai/dsh-robot-lab'
...
if (backend === 'rlx' && (!process.env.DSH_MICRODUCK_RLX_PYTHON || !process.env.DSH_MICRODUCK_RLX_SOURCE_ROOT)) throw new Error('RLX smoke requires its configured interpreter and source; no CPU fallback')
...
try {
  ...
  if (agents === undefined || lab === undefined) throw new Error('Fixture requires the real agent registry and Robot Lab')
  const { agent } = await agents.create({ sessionId: SessionId('robot-provider-smoke'), meta: { cwd: process.cwd() } })
  ...
  const movingJointIndices = profile.joints.map((_, index) => index).filter((index) => {
    const values = saved.project.clip.keys.map(key => key.joints[index]!)
    return Math.max(...values) - Math.min(...values) >= 0.01
  })
  if (dance && movingJointIndices.length === 0) throw new Error('Dance smoke requires an active authored target')
  // Prospective smoke criteria exercise the assessment path, not a qualified beginner preset.
  const danceCriteria: RobotDanceCriteria = { version: 1, requiredCycles: 2, minPassedEpisodeFraction: 1,
    maxJointRmseRad: Array<number>(14).fill(0.2), maxRootOrientationRmseRad: 0.5,
    movingJointIndices, minReferenceExcursionRad: 0.005, minAmplitudeRatio: 0.25,
    maxAmplitudeRatio: 2, minReferenceGainRatio: 0.25, maxHorizontalDriftMeters: 0.5 }
  const trial = await lab.execute(agent, {
    ...
    recipe: {
      ...
      evaluation: { episodes: 2, stepsPerEpisode: dance ? 1000 : 50, seed: 23, maxTerminations: 0,
        minMeanUprightFraction: 0.5, ...(dance ? { dance: danceCriteria } : {}) },
  ...
  if (dance && (result.run.dancePlan === undefined
    || result.binding.dancePlanSha256 !== learningHash(result.run.dancePlan)
    || !isDeepStrictEqual(result.run.dancePlan.evaluation, trial.trial.recipe.evaluation))) {
    throw new Error('Dance assessment must be resolved and bound before training starts')
  }
  ...
  if (evaluation.evaluation.policyHash !== run.policySha256 || assessedPolicy !== run.policyId
    || !isDeepStrictEqual(criteria, trial.trial.recipe.evaluation)) {
    throw new Error('Evaluation must use the trial\'s frozen policy and criteria')
  }
  if (dance && (!isDeepStrictEqual(evaluation.evaluation.dancePlan, run.dancePlan)
    || evaluation.evaluation.danceStatus === undefined || evaluation.evaluation.episodes.some(episode => episode.dance === undefined))) {
    throw new Error('Saved dance evidence must retain the pre-training plan and independently computed verdict')
  }
  ...
  const reflection = await lab.execute(agent, { operation: 'save_reflection', reflection: {
    ...
    observation: `Balance criteria passed: ${String(evaluation.evaluation.passed)}. Choreography: ${evaluation.evaluation.danceStatus ?? 'not assessed'}.`,
  ...
  process.stdout.write(JSON.stringify({
    mode: 'real-host-provider', backend, learnerDevice, trainingSteps, dance, readiness: readiness.readiness,
