...
把一个 [Claude Code](https://docs.claude.com/en/docs/claude-code) 插件——它的技能、命令、子代理、MCP 服务器与 hooks——作为一个 [profile bundle](../README.zh.md) 原生运行在 DeepSeek Harness 上。该 bundle 在 [`dsh-base`](../base/README.zh.md) 之上组合已发布的插件;它唯一依赖的 DSH 核心改动是新增的 [`mcp-client` `allowedTools`](../../mcp/mcp-client/README.zh.md) 字段。设计决策见 [Agent Note](../../../.agents/notes/implemented/feature/2026-08-29-claude-code-plugin-native-support.zh.md)。
...
- **Bundle 补丁(`cordis.patch.yml`)——静态、由环境驱动的默认值,以 `CC_PLUGIN_ROOT` 为键:**
  - `cc-skills` → [`skill-filesystem`](../../skill/skill-filesystem/README.zh.md),`customSkillDirs: [<root>/skills]`
  - `cc-commands` → [`cc-commands`](../../hooks/cc-commands/README.zh.md),带 `pluginRoot`
  - `cc-hooks` → [`hooks-claude-code`](../../hooks/hooks-claude-code/README.zh.md),除非 `<root>/hooks/hooks.json` 存在,否则 `disabled`
- **Profile 补丁(生成)——具体插件配置与逐插件行:** `scripts/install.mjs` 用技能、命令及 hooks 的字面路径覆盖 bundle 默认值,并为每个 `.mcp.json` 服务器生成一条 [`mcp-client`](../../mcp/mcp-client/README.zh.md) 行,为每个 `agents/*.md` 生成一条 [`tool-subagent`](../../subagent/tool-subagent/README.zh.md) 行。`scripts/cc-manifest.mjs` 为多个插件生成相应的命名空间行。
