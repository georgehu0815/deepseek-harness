...
import {
  ...
  DETAILS_DEFAULT, DETAILS_MIN, VISUAL_DEFAULT, VISUAL_MIN,
...
describe('computeColumns (visual closed — the three-column baseline)', () => {
  it('step 1: everything fits at preferred widths', () => {
    const cols = computeColumns(1920, open(SIDEBAR_DEFAULT), open(DETAILS_DEFAULT), closed(VISUAL_DEFAULT))
    expect(cols).toEqual({ sidebar: 280, center: 1920 - 280 - 360, details: 360, visual: 0 })
  })
  ...
  it('closed sidebar keeps its compact rail while closed details contribute zero width', () => {
    expect(computeColumns(1920, closed(300), closed(360), closed(0)))
      .toEqual({ sidebar: SIDEBAR_COLLAPSED, center: 1920 - SIDEBAR_COLLAPSED, details: 0, visual: 0 })
  })
  ...
  it('step 2: details shrinks first, center pinned at min', () => {
    ...
    expect(cols).toEqual({ sidebar: 280, center: CENTER_MIN, details: 330, visual: 0 })
  })

  it('boundary: exactly at the step-1/step-2 seam', () => {
    const cols = computeColumns(300 + 360 + CENTER_MIN, open(300), open(360), closed(0))
    expect(cols).toEqual({ sidebar: 300, center: CENTER_MIN, details: 360, visual: 0 })
    const one = computeColumns(300 + 360 + CENTER_MIN - 1, open(300), open(360), closed(0))
    expect(one).toEqual({ sidebar: 300, center: CENTER_MIN, details: 359, visual: 0 })
  })
  ...
  it('details auto-closes when its min still starves center — sidebar holds its preference', () => {
    ...
    expect(cols).toEqual({ sidebar: 280, center: 930, details: 0, visual: 0 })
  })
  ...
  it('the sidebar never concedes: center absorbs the deficit below CENTER_MIN', () => {
    ...
    expect(cols).toEqual({ sidebar: SIDEBAR_DEFAULT, center: 420, details: 0, visual: 0 })
  })
  ...
  it('sidebar-closed narrow window: details concedes then auto-closes', () => {
    ...
    expect(fits).toEqual({ sidebar: SIDEBAR_COLLAPSED, center: CENTER_MIN, details: DETAILS_MIN, visual: 0 })
...
describe('computeColumns — the visual column concedes ahead of details', () => {
  it('step 1: all four fit at preferred widths', () => {
    // 280 + 640 + 360 + 480 = 1760 <= 1920.
    const cols = computeColumns(1920, open(SIDEBAR_DEFAULT), open(DETAILS_DEFAULT), open(VISUAL_DEFAULT))
    expect(cols).toEqual({ sidebar: 280, center: 1920 - 280 - 360 - 480, details: 360, visual: 480 })
  })

  it('step 2: visual shrinks first while details stays at its preference', () => {
    // 280 + 640 + 360 + 480 = 1760 > 1720; visual concedes to 1720-280-360-640 = 440.
    const cols = computeColumns(1720, open(SIDEBAR_DEFAULT), open(DETAILS_DEFAULT), open(VISUAL_DEFAULT))
    expect(cols).toEqual({ sidebar: 280, center: CENTER_MIN, details: 360, visual: 440 })
  })

  it('step 3: visual pins at its min, then details shrinks', () => {
    // visual floored at VISUAL_MIN=360; then details concedes.
    // viewport chosen so visual is already at min and details must give.
    const viewport = SIDEBAR_DEFAULT + CENTER_MIN + DETAILS_DEFAULT + VISUAL_MIN - 20
    const cols = computeColumns(viewport, open(SIDEBAR_DEFAULT), open(DETAILS_DEFAULT), open(VISUAL_DEFAULT))
    expect(cols.visual).toBe(VISUAL_MIN)
    expect(cols.center).toBe(CENTER_MIN)
    expect(cols.details).toBe(DETAILS_DEFAULT - 20)
    expect(cols.sidebar).toBe(SIDEBAR_DEFAULT)
  })

  it('step 4: visual auto-closes when its min still starves center; details survives, absorbing the freed room up to its clamp', () => {
    // viewport 1260: sidebar 280 + center-min 640 leaves 340 for the extra
    // columns. visual (min 360) cannot fit, so it auto-closes; details then
    // takes the freed room (340, above its 300 min, below its 520 max).
    const viewport = SIDEBAR_DEFAULT + CENTER_MIN + DETAILS_MIN + 40
    const cols = computeColumns(viewport, open(SIDEBAR_DEFAULT), open(DETAILS_DEFAULT), open(VISUAL_DEFAULT))
    expect(cols).toEqual({ sidebar: SIDEBAR_DEFAULT, center: CENTER_MIN, details: 340, visual: 0 })
  })

  it('step 5: both visual and details auto-close; center absorbs the remaining deficit', () => {
    // Not even sidebar + center-min + details-min fits: both extra columns close.
    const viewport = SIDEBAR_DEFAULT + CENTER_MIN + 100
    const cols = computeColumns(viewport, open(SIDEBAR_DEFAULT), open(DETAILS_DEFAULT), open(VISUAL_DEFAULT))
    expect(cols).toEqual({ sidebar: SIDEBAR_DEFAULT, center: viewport - SIDEBAR_DEFAULT, details: 0, visual: 0 })
  })

  it('visual open with details closed: visual concedes to its min, then auto-closes with center pinned at its min', () => {
    const fits = computeColumns(
      SIDEBAR_DEFAULT + CENTER_MIN + VISUAL_MIN,
      open(SIDEBAR_DEFAULT), closed(DETAILS_DEFAULT), open(VISUAL_DEFAULT),
    )
    expect(fits).toEqual({ sidebar: SIDEBAR_DEFAULT, center: CENTER_MIN, details: 0, visual: VISUAL_MIN })
    // One px short of the visual min fitting: visual auto-closes and, with room
    // for sidebar + center-min, center holds at CENTER_MIN (step 4).
    const starved = computeColumns(
      SIDEBAR_DEFAULT + CENTER_MIN + VISUAL_MIN - 1,
      open(SIDEBAR_DEFAULT), closed(DETAILS_DEFAULT), open(VISUAL_DEFAULT),
    )
    expect(starved).toEqual({ sidebar: SIDEBAR_DEFAULT, center: CENTER_MIN, details: 0, visual: 0 })
  })

  it('recovery is pure for visual too: re-widening restores its preference', () => {
    const squeezed = computeColumns(1000, open(SIDEBAR_DEFAULT), open(DETAILS_DEFAULT), open(VISUAL_DEFAULT))
    expect(squeezed.visual).toBe(0)
    const restored = computeColumns(1920, open(SIDEBAR_DEFAULT), open(DETAILS_DEFAULT), open(VISUAL_DEFAULT))
    expect(restored.visual).toBe(VISUAL_DEFAULT)
    expect(restored.details).toBe(DETAILS_DEFAULT)
  })

  it('visual preference beyond the clamp range is clamped before solving', () => {
    const cols = computeColumns(3000, open(SIDEBAR_DEFAULT), open(DETAILS_DEFAULT), open(9999))
    expect(cols.visual).toBe(720) // VISUAL_MAX
  })
})
...
describe('computeColumns — degenerate viewports', () => {
  it('sidebar closed and viewport below CENTER_MIN: details auto-closes, center takes the rest', () => {
    ...
    expect(computeColumns(500, closed(300), open(DETAILS_DEFAULT), closed(0)))
      .toEqual({ sidebar: SIDEBAR_COLLAPSED, center: 500 - SIDEBAR_COLLAPSED, details: 0, visual: 0 })
  })
})
