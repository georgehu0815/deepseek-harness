...
在对话视图环中打开 Trajectory 标签页，把 agent 活动作为事件记录表与时间线查看。新会话在发送第一条消息前即可使用该标签页；默认仍选中 Chat，记录到达前 Trajectory 不显示计时数据。初始尾部完成定位前，记录表会用明确的加载行遮住真实记录；更早的前缀仍未加载时，首行控件会在点击时加载一页更早的历史，并在该页加载期间显示禁用的加载状态。
