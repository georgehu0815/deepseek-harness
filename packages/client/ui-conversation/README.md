...
`UiConversation.events` is the single registry for event Definitions, and `UiConversation.views` is the single registry for event-backed builders and presentation-only View capabilities. Both registries reject duplicate keys, preserve registration order, return idempotent disposers, and rebuild existing bindings when their contribution roster changes. `UiConversation.binding(bindingOrSessionId)` returns one identity-stable Conversation binding for the current Session Controller binding. It does not open another event source.
...
An event-backed target becomes active when shell selection resolves it or when its source receives a first subscriber. The assembler replaces that target from current Contexts once and keeps it active for later incremental flushes; creating a source does not activate it and unsubscription does not deactivate it.
...
The shell reads the persisted View preference before rendering when a Session first binds or a cached Session becomes current, activates the registered preferred View or Chat fallback, and activates later tab or focus selections before committing them to the store. Blank-session navigation admits registered Chat and Views whose matching definition explicitly sets `supportsBlankSession: true`; Chat has no blank View body. An eligible selected non-Chat View renders its controls without changing Session lifecycle or Conversation activity. A real blank Session with an opted-in non-Chat View exposes compact workspace navigation; no-Session composition remains unchanged. `presentationOnly: true` skips event builders, snapshots and activity contribution, independently of blank-session eligibility. The [Conversation reference](../../../docs/subsystems/conversation.md#blank-session-presentation) defines both fields; the [presentation decision](../../../.agents/notes/implemented/architecture/2026-09-05-blank-session-presentation-views.md) records why neither belongs in SlotCore.

`ctx.uiConversation.selectView(sessionId, target)` selects an installed eligible View for the current known Session; unlike `binding.activate`, it changes UI selection rather than only activating target data. The owner delivers the command through the actual declared View store's action after its layout mount. Before that mount, only the latest one-shot command waits; no second selection state is maintained. Session/binding changes, target withdrawal or loss of eligibility, newer owner navigation, body unmount and plugin disposal cancel undelivered commands. The required `UiConversation` constructor navigation factory runs synchronously after registry initialization. [The navigation decision](../../../.agents/notes/implemented/architecture/2026-09-05-conversation-owned-view-commands.md) records the ownership and cancellation rules.

The resident composer stays at the bottom of the conversation viewport, including the welcome screen and blank Chat with no View body, and survives no-Session and Session transitions. The no-Session state keeps the same composer surface mounted but inert while the Workspace picker connects a blank Session. The surface is a shell-owned Lexical editor: reference chips are atomic decorator nodes carrying the owner's serialization identity (submission expands them through the owner codec), claimed slash commands stay styled leading text, folder text references carry the folder glyph as an icon prefix, and the draft's clipboard projection is mirrored into the per-Session Conversation store. Queue operations address exact queue occurrences through the scoped `ctx.conversation` service; queue previews render sent text through the shared inline reference projection from `ui-primitives` (wire session forms fold to their label) and show local or durable images and files in original attachment order. Images use thumbnails; files use compact name-and-size cards. An edit exposes the literal sent text, and durable thumbnails resolve through the session image URL cache. Busy Enter behavior is stored in the Host-backed `ui-conversation` settings namespace.
