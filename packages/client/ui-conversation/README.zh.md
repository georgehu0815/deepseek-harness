...
shell 选择解析出事件驱动 target 或其 source 收到首个 subscriber 时，该 target 进入 active 状态。assembler 从当前 Context 对它执行一次 replace，并使它参与后续增量 flush；创建 source 不会激活 target，取消订阅也不会停用 target。
...
Session 首次绑定或缓存的 Session 成为 current 时，shell 会在渲染前读取持久化 View 偏好，激活已注册的偏好 View 或 Chat fallback，并在后续 tab 或 focus 选择写入 store 前先激活对应 target。空白会话导航允许已注册的 Chat，以及匹配 Definition 显式设置 `supportsBlankSession: true` 的 View；Chat 没有空白 View body。选中的合格非 Chat View 渲染自身控件，不改变 Session lifecycle 或 Conversation activity。真实空白 Session 存在显式启用的非 Chat View 时提供紧凑工作区导航；无 Session 组合保持不变。`presentationOnly: true` 跳过 event builder、snapshot 与 activity 贡献，与空白会话资格相互独立。[Conversation 参考](../../../docs/subsystems/conversation.zh.md#blank-session-presentation)定义两个字段；[呈现决策](../../../.agents/notes/implemented/architecture/2026-09-05-blank-session-presentation-views.zh.md)记录两者都不属于 SlotCore 的理由。

`ctx.uiConversation.selectView(sessionId, target)` 为当前已知 Session 选择已安装且具备资格的 View；与 `binding.activate` 不同，它改变 UI 选择，而不只是激活 target 数据。Owner 在布局挂载后，通过实际声明的 View store action 交付命令。挂载前只等待最新的一次性命令；不维护第二套选择状态。Session 或 binding 变化、target 撤回或失去资格、更新的 owner 导航、body 卸载及插件释放会取消尚未交付的命令。必需的 `UiConversation` 构造器导航工厂在 registry 初始化后同步运行。[导航决策](../../../.agents/notes/implemented/architecture/2026-09-05-conversation-owned-view-commands.zh.md)记录所有权与取消规则。

常驻 composer 保持在会话视口底部，包括欢迎屏幕和没有 View body 的空白 Chat，并在无 Session 与有 Session 之间保持挂载。无 Session 时，同一个编辑器表面保持 inert，Workspace picker 连接 blank Session。该表面是 shell 所有的 Lexical 编辑器：引用 chip 是携带 owner 序列化身份的原子 decorator 节点（提交时经 owner codec 展开），已认领的 slash command 保持为带样式的行首文本，文件夹文本引用以图标前缀携带文件夹图形，草稿的剪贴板投影镜像到逐 Session Conversation store。Queue 操作通过 scoped `ctx.conversation` service 寻址准确的 queue occurrence；queue 预览经 `ui-primitives` 的共享行内引用投影渲染已发送文本（wire 会话形式折叠为其标签），并按原始附件顺序展示本地或持久化的图片和文件。图片使用缩略图，文件使用紧凑的名称与大小卡片。编辑态展示字面发送文本，持久化缩略图通过会话图片 URL 缓存解析。繁忙时 Enter 行为保存在 Host-backed `ui-conversation` settings namespace。
