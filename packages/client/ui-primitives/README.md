...
Markdown links to absolute `http://` or `https://` URLs without embedded credentials gain native audio controls when the URL pathname ends in `.mp3` (case-insensitive); query strings and fragments are allowed. Direct links, resolved reference links, and autolinks retain their original named link beside the player. Audio does not autoplay and uses `preload="none"`; raw HTML remains disabled, and URLs inside inline code do not gain players ([audio decision](../../../.agents/notes/implemented/feature/2026-09-07-web-inline-markdown-audio.md)).
...
- **Remote audio depends on the browser and origin** — the `.mp3` suffix does not prove playable media. The original link remains available when loading or decoding fails. `preload="none"` is a browser hint, not a guarantee of no network requests; playback uses the remote URL directly, without a host proxy.
