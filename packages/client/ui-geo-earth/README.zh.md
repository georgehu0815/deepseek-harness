...
交互式 3D 地球插件：侧边栏底部的“Earth 3D”动作会在外壳一等的 `earth` 网格列——第四条 `ui-layout` 轨道（`sidebar | center | details | earth`）——中打开一个 CesiumJS 地球。按钮通过 `ctx.layout.toggleEarth()` 切换该列，列头通过 `ctx.layout.closeEarth()` 关闭它，因此地球是一个真实的、可缩放的布局列，而非漂浮浮层。契约：[槽位系统标准](../../../.agents/notes/implemented/architecture/2026-07-22-slot-type-chain-implementation.zh.md)。
