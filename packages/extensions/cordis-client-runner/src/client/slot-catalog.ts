...
export const CLIENT_SLOT_API: readonly ClientSlotEntry[] = [
  ...
  {
    key: 'conversation.micro-duck.learning-plan',
    kind: 'single',
    scope: 'session',
    summary: 'Session-owned trial planning section; the parent passes no owner arguments.',
    doc: 'Session-owned trial planning section; the parent passes no owner arguments.',
    registerOptions: [],
    ownerProps: [],
    ownerPropsReferences: [],
    standardProps: [
    ...
    hookContext: '',
    slotInject: '',
    declaredBy: 'an entry in \'conversation.view\' (client-ui-robot-lab), so it exists while that entry is mounted',
    occupants: [
      'client-ui-robot-lab LearningPlan',
    ],
    replaceRisk: 'shadows-shipped-ui',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'conversation.micro-duck.learning-plan\', () => ctx.slots.register(\n      { name: \'conversation.micro-duck.learning-plan\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
    source: 'packages/client/ui-robot-lab/src/client/studio-props.ts:22',
  },
  {
    key: 'conversation.micro-duck.learning-review',
    kind: 'single',
    scope: 'session',
    summary: 'Session-owned assessment and reflection section; the parent passes no owner arguments.',
    doc: 'Session-owned assessment and reflection section; the parent passes no owner arguments.',
    registerOptions: [],
    ownerProps: [],
    ownerPropsReferences: [],
    standardProps: [
    ...
    declaredBy: 'an entry in \'conversation.view\' (client-ui-robot-lab), so it exists while that entry is mounted',
    occupants: [
      'client-ui-robot-lab LearningReview',
    ],
    replaceRisk: 'shadows-shipped-ui',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'conversation.micro-duck.learning-review\', () => ctx.slots.register(\n      { name: \'conversation.micro-duck.learning-review\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
    source: 'packages/client/ui-robot-lab/src/client/studio-props.ts:24',
  },
  {
  ...
  },
  {
    ...
    declaredBy: 'an entry in \'conversation.session.header\' (client-ui-conversation), so it exists while that entry is mounted',
    occupants: [
      'client-ui-geo-earth GeoCommandBridge id \'geo-earth-3d-bridge\'',
      'client-ui-geo-earth GeoViewReporter id \'geo-earth-3d-reporter\'',
  ...
  },
  {
    ...
    occupants: [
      ...
      'client-ui-geo-earth SummaryTable id \'geo-summary\'',
      'client-ui-robot-lab ClipGenPanel id \'clip-gen\'',
      'client-ui-robot-lab MicroDuckPanel id \'micro-duck\'',
      'client-ui-supply-chain SupplyChainPanel id \'supply-chain\'',
  ...
  {
    key: 'robot-lab.clip.player',
    kind: 'single',
    scope: 'session',
    summary: 'Session-scoped authored preview; the visual entry supplies no owner arguments.',
    doc: 'Session-scoped authored preview; the visual entry supplies no owner arguments.',
    registerOptions: [],
    ownerProps: [],
    ownerPropsReferences: [],
    standardProps: [
    ...
    hookContext: '',
    slotInject: '',
    declaredBy: 'an entry in \'visual.workspace.view\' (client-ui-robot-lab), so it exists while that entry is mounted',
    occupants: [
      'client-ui-robot-lab ClipSimulation',
    ],
    replaceRisk: 'shadows-shipped-ui',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'robot-lab.clip.player\', () => ctx.slots.register(\n      { name: \'robot-lab.clip.player\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
    source: 'packages/client/ui-robot-lab/src/client/clip-gen-props.ts:36',
  },
  {
    key: 'robot-lab.visual.player',
    kind: 'single',
    scope: 'session',
    summary: 'Provide the session player inside Robot Studio; the owner passes no arguments.',
    doc: 'Provide the session player inside Robot Studio; the owner passes no arguments.\nThe component receives session-scoped runtime props plus its declared store and injections.\nThe selected registration replaces the default player; no registration leaves the player area empty.',
    registerOptions: [],
    ownerProps: [],
    ownerPropsReferences: [],
    standardProps: [
    ...
    declaredBy: 'an entry in \'visual.workspace.view\' (client-ui-robot-lab), so it exists while that entry is mounted',
    occupants: [
      'client-ui-robot-lab StudioPlayer',
    ],
    replaceRisk: 'shadows-shipped-ui',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'robot-lab.visual.player\', () => ctx.slots.register(\n      { name: \'robot-lab.visual.player\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
    source: 'packages/client/ui-robot-lab/src/client/studio-props.ts:20',
  },
  {
    key: 'root',
    kind: 'single',
  ...
  },
  {
    ...
    declaredBy: 'an entry in \'root\' (client-ui-layout), so it exists while that entry is mounted',
    occupants: [
      'client-ui-anatomy-3d AnatomyOverlay id \'human-body-3d-panel\'',
    ],
    replaceRisk: 'none',
  ...
  },
  {
    ...
    declaredBy: 'an entry in \'sidebar\' (client-ui-sidebar), so it exists while that entry is mounted',
    occupants: [
      'client-ui-anatomy-3d AnatomySidebarButton id \'human-body-3d\'',
      'client-ui-geo-earth EarthSidebarButton id \'geo-earth-3d\'',
      'client-ui-robot-lab SidebarButton id \'robot-lab\'',
  ...
  {
    key: 'visual.workspace.view',
    kind: 'list',
    scope: 'session-maybe',
    summary: 'Plugin-contributed visual workspace tabs.',
    doc: 'Plugin-contributed visual workspace tabs. The shell renders the selected entry only.',
    registerOptions: [
      {
        name: 'id',
        requirement: 'required',
    ...
    declaredBy: 'an entry in \'root\' (client-ui-layout), so it exists while that entry is mounted',
    occupants: [
      'client-ui-geo-earth EarthColumnPanel id \'earth\'',
      'client-ui-robot-lab ClipSimulationEntry id \'simulation-workspace\'',
      'client-ui-robot-lab RobotLab id \'robot-lab\'',
    ],
    replaceRisk: 'none',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'visual.workspace.view\', () => ctx.slots.register(\n      { name: \'visual.workspace.view\', id: \'my-entry\', order: 100, label: \'My entry\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
