...
生成的 [tool-supply-chain schemas](../../../docs/tool-catalog.md#deepseek-aidsh-tool-supply-chain)。在目录已说明之外：每个数值参数的描述都以 `Accepted range {min} to {max}.` 结尾，且 `preset` 接受 `baseline`、`demandShock`、`disruption`、`lowCapacity` 或 `thinSafetyStock`。`disruptionEdge` 是一条两元素的 `[source, target]` 线路，且只有与非零的 `disruptionProbability` 同时给出时才生效。
