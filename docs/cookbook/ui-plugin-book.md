---
title: DeepSeek Harness UI Plugin Student Book
description: Build a production-ready DeepSeek Harness UI plugin from a temporary Studio prototype through Host tools, Client slots, ACP, SDK integration, testing, and installation
author: George Hu
ms.date: 2026-08-23
ms.topic: tutorial
keywords:
...
English | [中文](ui-plugin-book.zh.md)
...
Complete [Your first Harness plugin](../user/develop/basic/index.md) before starting. Use Node.js `^22.19` or `>=24`, pnpm, and a local checkout with `pnpm install` complete.
...
Read [Tool authoring](adding-a-tool.md) for the complete Tool contract and [Add a conversation node](adding-a-conversation-node.md) when the UI represents a durable multi-event workflow rather than one Tool call.
...
Create the package outside this repository when you want an independently installable plugin. During repository development, place an official package under the appropriate `packages/<group>/<name>/` directory and follow [Add a workspace package](adding-a-package.md).
...
For an in-repository official Client package, add its package name to the Web bundle dependencies and add a `dsh.client` row to [`packages/bundle/web-app/cordis.patch.yml`](../../packages/bundle/web-app/cordis.patch.yml). Add the package to `tsconfig.client.json` as described by [the workspace package checklist](adding-a-package.md#2-register-it-in-the-root-configs).
...
Treat [the ACP package contract](../../packages/acp/acp/README.md) as authoritative. Do not advertise a Studio-only interaction as portable to ACP.
...
Do not parse the assistant's prose to recover identifiers or progress. Prefer canonical Tool values, durable metadata, or dedicated Session events. [The TypeScript SDK](../../packages/sdk/client/README.md), [Python SDK](../../python/sdk/README.md), and [JSON-RPC example](../../examples/jsonrpc-agent/README.md) define the supported process lifecycle and event access.
...
Use [`ui-user-questions` browser plugin tests](../../packages/client/ui-user-questions/tests/browser-plugin.client.spec.ts) as the compact reference and [`ui-cordis`](../../packages/extensions/ui-cordis/src/client/index.ts) as a multi-Slot production example.
...
For an official in-repository package, also run the selected typecheck, lint, package constraints, documentation checks, and pre-push checks required by the changed files. The [testing policy](../testing.md) defines when unit, coverage, real-API, snapshot, and browser tests are required.
...
* [Web Client package rules](../../packages/client/AGENTS.md) for Slot, props, state, dependency, styling, and test constraints
* [Web styling](../web-styling.md) for tokens and CSS Modules
* [Tool authoring](adding-a-tool.md) for schemas, execution, model rendering, durable metadata, and neutral UI intents
* [Conversation Nodes](adding-a-conversation-node.md) for replayable business rows
* [Package installation](../user/develop/basic/publish.md) for bundles and profiles
* [Architecture](../architecture.md) for plugin composition and durable Session events
* [ACP](../../packages/acp/acp/README.md) for editor automation limits
* [TypeScript SDK](../../packages/sdk/client/README.md) and [Python SDK](../../python/sdk/README.md) for external application integration
