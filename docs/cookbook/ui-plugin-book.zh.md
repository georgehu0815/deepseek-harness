...
开始前，请先完成[第一个 Harness 插件](../user/develop/basic/index.zh.md)。请使用 Node.js `^22.19` 或 `>=24`、pnpm，并在本地检出中完成 `pnpm install`。
...
请阅读[工具编写](adding-a-tool.zh.md)了解完整的工具约定。当 UI 表示持久的多事件工作流，而不是单次工具调用时，请阅读[添加会话节点](adding-a-conversation-node.md)。
...
如果需要独立安装的插件，请在本仓库之外创建包。在仓库内开发官方包时，请将其放在适当的 `packages/<group>/<name>/` 目录下，并遵循[添加工作区包](adding-a-package.zh.md)。
...
对于仓库内的官方 Client 包，请把包名添加到 Web 组合包依赖中，并向 [`packages/bundle/web-app/cordis.patch.yml`](../../packages/bundle/web-app/cordis.patch.yml) 添加 `dsh.client` 配置项。按照[工作区包检查清单](adding-a-package.zh.md#2-register-it-in-the-root-configs)的说明，将该包添加到 `tsconfig.client.json`。
...
通过 Git 安装时，包需要自包含的 `prepare` 脚本。pnpm 10 要求用户在 profile 的 `pnpm-workspace.yaml` 中允许该包执行构建。如果不希望在安装时执行代码，请优先使用包含已构建产物的注册表包或 tarball。[打包并安装插件](../user/develop/basic/publish.zh.md)定义了完整的 profile、组合包、Git 和信任工作流。
...
请以 [ACP 包约定](../../packages/acp/acp/README.zh.md)为准。不要把仅限 Studio 的交互描述为可移植到 ACP。
...
不要解析助手正文来恢复标识符或进度。请优先使用规范的工具值、持久元数据或专用会话事件。[TypeScript SDK](../../packages/sdk/client/README.zh.md)、[Python SDK](../../python/sdk/README.zh.md)和 [JSON-RPC 示例](../../examples/jsonrpc-agent/README.md)定义了受支持的进程生命周期与事件访问方式。
...
对于仓库内的官方包，还要运行改动文件要求的类型检查、lint、包约束、文档检查和推送前检查。[测试政策](../testing.zh.md)定义了何时需要单元测试、覆盖率测试、真实 API 测试、快照测试和浏览器测试。
...
* [Web 样式](../web-styling.zh.md)，包含 token 和 CSS Module 规则
* [工具编写](adding-a-tool.zh.md)，包含 schema、执行、模型渲染、持久元数据和通用 UI 意图
* [会话节点](adding-a-conversation-node.md)，用于可回放的业务行
* [包安装](../user/develop/basic/publish.zh.md)，包含组合包和 profile 规则
* [架构](../architecture.zh.md)，包含插件组合和持久会话事件
* [ACP](../../packages/acp/acp/README.zh.md)，包含编辑器自动化限制
* [TypeScript SDK](../../packages/sdk/client/README.zh.md)和 [Python SDK](../../python/sdk/README.zh.md)，用于外部应用集成
