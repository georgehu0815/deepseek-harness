...
🔨 **Step 3 — launch the generated profile:**

```sh
pnpm dsh --profile windows-brain --port 3080
```
...
# 4. run — the generated profile already contains the plugin paths
...
- Edited the plugin's **`commands/` or `skills/`** → nothing to regenerate; the generated profile stores their directories and reads their contents at boot. Restart with `--no-build`.
