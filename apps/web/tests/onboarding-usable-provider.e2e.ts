...
// for the Agency Copilot token again, and the first-run setup card is a card
// they can close. The shipped Agency Copilot adapter stays mounted without a
...
describe.skipIf(MODE === 'record')('web e2e: another usable provider ends first-run onboarding', () => {
  ...
  it('dismisses setup without blocking the add-provider workflow', async () => {
    ...
    // Dismissing first-run setup leaves the normal provider draft usable.
    expect(await settings.getByLabel('提供方').count()).toBe(1)
    await settings.getByRole('button', { name: '编辑 Agency Copilot (agency-copilot)' }).waitFor({ timeout: 10_000 })
  ...
  it('stops prompting for Agency Copilot once the other provider can serve requests', async () => {
    ...
    // Only minimax-cn is reachable; Agency Copilot still holds no credential.
    ...
    expect(credentials).not.toContain('CLAUDE_CODE_COPILOT_TOKEN')
    ...
    // The Models page agrees: Agency Copilot stays a row rather than reopening its
